...
const Reviews = (revData) => {
    ...
    useEffect(()=> {
        let newErrors = []
        if(revRating > 5 || revRating < 1) newErrors.push('Rating must be between 1 and 5');
        if(revTitle.length < 2) newErrors.push('Review title must be at least 2 characters');
        if(revTitle.length > 50) newErrors.push('Review title can not exceed 50 characters');
        if(revContent.length < 2) newErrors.push('Review title must be at least 2 characters');
        if(revContent.length > 400) newErrors.push('Review title can not exceed 400 characters');
                ...
                <div className="review-container" hidden={editHidden}>
                    <form action={`/items/${itemId}/new-review`} method='patch'>
                        ...
                        {revEditErrors.map((e, i) => (
                            <p hidden={hideRevEditErrors} className='error' key={i}>{e}</p>
                        ))}
                        <div className='flex-col-cont'>
                            <label className='grey-label'><span hidden={hideRevEditErrors} className='req-star'> *</span> Score</label>
                            <input type='number'
                        ...
                        <div className='flex-col-cont'>
                            <label className='grey-label'><span hidden={hideRevEditErrors} className='req-star'> *</span> Title</label>
                            <input type='text'
                            className='rev-input'
                                value={revTitle}
                        ...
                        <div className='flex-col-cont'>
                            <label className='grey-label'><span hidden={hideRevEditErrors} className='req-star'> *</span> Review</label>
                            <textarea type='text'
                            className='rev-input'
                                value={revContent}
                    ...
                    <div className='review-option-buttons flex-row-cont'> 
                        <button className='grey-green-btn' onClick={e=>handleRevEdit(e)}>Submit</button>
