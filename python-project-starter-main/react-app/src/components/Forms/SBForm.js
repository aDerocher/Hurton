...
const ItemForm = () => {
  ...
  const addItemToWishlist = (e, item) => {
    ...
    if (itemExistsInWishlist(usersWishlist, item)) {
      return;
    }
    ...
    if (uid) {
      const existingCartItem = usersCart.filter((item) => {
        if (
          ...
          item.item_size === formData.item_size &&
          item.item_color === formData.item_color
        ) {
          return true;
        } else {
          return false;
      ...
      if (existingCartItem.length > 0) {
        ...
        dispatch(
          editCartItem(existingCartItem[0].id, existingCartItem[0].quantity),
        );
        setShowCCModal(true);
        return;
      }
  ...
  const addToLocalCart = (data) => {
    ...
    if (cart[`${data.item_id}_${data.item_color}_${data.item_size}`]) {
      cart[`${data.item_id}_${data.item_color}_${data.item_size}`].quantity +=
        data.quantity;
    } else {
  ...
  return (
    <div className="item-form-container">
      ...
      <div className="form-stars-row flex-row-cont">
        {five.map((n, i) => (
          <div key={i} className="">
            {n <= score && <i className="fas fa-star"></i>}
            {n > score && <i className="far fa-star"></i>}
          </div>
        ))}
        <p className="grey-label reviews-counter">
          {" "}
          {itemReviews?.length} Reviews
        </p>
      </div>
      {curItem?.item_type === 1 && <p className="">{color}</p>}
      ...
      <form className="item-selection-form-inputs">
        ...
        <select
          ...
          id=""
        >
          <option className="def-option" disabled defaultValue="1">
            1
          </option>
      ...
      <div className="flex-col-cont item-form-buttons-cont">
        <div className="flex-row-cont item-form-buttons-subcont">
          <button
            className="grey-green-btn item-form-submit-btn"
          ...
          {curItem && sessionUser && (
            <button
              className="item-form-wl-btn dis"
              disabled={
                itemExistsInWishlist(usersWishlist, curItem) || !sessionUser
              }
              onClick={(e) => {
                e.stopPropagation();
                addItemToWishlist(e, curItem);
              }}
            >
          ...
          {!sessionUser && (
            <button
              className="item-form-wl-btn2 dis"
