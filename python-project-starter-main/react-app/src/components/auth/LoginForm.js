...
const LoginForm = () => {
  ...
  useEffect(() => {
    ...
    if (email.length > 250)
      newErrorsE.push("Email cannot exceed 250 characters");
    if (!email.includes("@") || !email.includes("."))
      newErrorsE.push("Invalid email address");
    if (password.length > 24)
      newErrorsP.push("Password cannot exceed 25 characters");
  ...
  return (
    <div className="login-form-container">
      <form onSubmit={onLogin}>
        ...
        <button
          type="submit"
          className="auth-btn grey-green-btn"
          disabled={!hideErrors && errors.length > 0}
        >
          SIGN IN
        </button>
        <p>
