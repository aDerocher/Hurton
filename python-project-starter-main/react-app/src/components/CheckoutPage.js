...
const CheckoutPage = (subtotalParams) => {
  ...
  // Check for errors ====================================
  useEffect(() => {
    if (firstName && lastName && email && card && address) {
      setHideErr(false);
    }
    let newErrors = [];
    if (!email?.includes("@") || !email.includes("."))
      newErrors.push("Invalid Email Address: must include '@' and '.'");
    ...
    if (firstName?.length < 2)
      newErrors.push("First name must be longer than 2 characters");
    if (firstName?.length > 50)
      newErrors.push("First name must be less than 50 characters");
    if (lastName?.length < 2)
      newErrors.push("Last name must be longer than 2 characters");
    if (lastName?.length > 50)
      newErrors.push("Last name must be less than 50 characters");
    if (!parseInt(card) || parseInt(card).toString().length < 15)
      newErrors.push("Invalid Card Number: Invalid characters");
    if (card.length > 15)
      newErrors.push("Invalid Card Number: Number too long");
    if (card.length < 15)
      newErrors.push("Invalid Card Number: Number not long enough");
    if (address?.length < 4 || address === null)
      newErrors.push("Invalid Address: Address is too short");
    if (address?.length > 50 || address === null)
      newErrors.push("Invalid Address: Address is too long");
    setErrors(newErrors);
  }, [dispatch, firstName, lastName, address, email, card]);

  return (
    <div className="content-width">
      <div className="checkout-top-title flex-row-cont">
        <h1>Secure Checkout</h1>
        <i className="big-lock fas fa-lock"></i>
      </div>
      <div className="checkout-page-container flex-row-cont">
        <form className="checkout-left">
          ...
          <br />
          <p className="grey-label" hidden={hideErr}>
            <span className="req-star">*</span> indicates required field
          </p>
          <div className="checkout-box email-confirm checkout-email">
            <span className="req-star" hidden={hideErr}>
              *
            </span>
            <input
              type="email"
          ...
          <p className="grey-label email-confirm-text">
            To confirm your order.
          </p>
          <div className="flex-row-cont check-signup">
            <input type="checkbox" />
            <p>
              Sign me up for promotion and product update emails (you can
          ...
          <div className="shipping-radio flex-row-cont">
            ...
            />
            <p>
              <b>FREE </b> : 1-2 Business Days (Hurton Special Delivery)
            </p>
          </div>
          <div className="shipping-radio flex-row-cont">
            <input type="radio" name="shipping-radio" value="77" />
            <p>
              <b>$77.00 </b> : 3-5 Business Days (Standard Air)
            </p>
          </div>
          <div className="shipping-radio flex-row-cont">
            <input type="radio" name="shipping-radio" value="100" />
            <p>
              <b>$100.00 </b> : 7-12 Business Days (Standard Ground)
            </p>
          </div>
          <div className="shipping-radio flex-row-cont">
            <input type="radio" name="shipping-radio" value="97" />
            <p>
              <b>$97.00 </b> : 4-6 Weeks (No Rush)
            </p>
          </div>
        ...
        <div className="checkout-right">
          <div className="checkout-brick checkout-brick-grey">
            <p>Your Order</p>
          </div>
